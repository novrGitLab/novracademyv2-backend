-- Net-new schema additions from merging feat/marketing-assessments-enrollment-codes
-- into main: newsletter/marketing campaigns, the assessment system, and
-- enrollment codes. Diffed against main's schema as of the merge (NOT a
-- full from-scratch schema) — assumes everything already in main's
-- schema.prisma (Organization, ComplianceSetting/ComplianceAssignment,
-- etc.) is already applied to the real database.
--
-- Idempotent and safe to re-run: every CREATE TYPE is wrapped in a DO block
-- that swallows a duplicate_object error (Postgres has no CREATE TYPE IF
-- NOT EXISTS). CREATE TABLE / CREATE INDEX use IF NOT EXISTS directly.
-- ADD CONSTRAINT is wrapped the same way as CREATE TYPE. ADD VALUE on the
-- existing EnrollmentSource enum uses IF NOT EXISTS (Postgres 12+).
--
