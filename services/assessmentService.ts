...
export async function listAssessmentsForAdmin(user: AuthUser) {
  ...
  return prisma.assessment.findMany({
    where: { scope: AssessmentScope.ORGANIZATION, organizationId: user.organizationId ?? "__none__" },
    orderBy: { createdAt: "desc" },
    include: { _count: { select: { questions: true, attempts: true } } },
  });
}
...
export async function listAssessmentsForLearner(user: AuthUser) {
  const scopeFilter = user.organizationId
    ? { OR: [{ scope: AssessmentScope.UNIVERSAL }, { scope: AssessmentScope.ORGANIZATION, organizationId: user.organizationId }] }
