...
/** Renders + uploads a certificate PDF for an already-created Certificate row, pulling branding from its course's Tenant. Shared by issuance and admin-triggered regeneration. */
async function renderAndUploadCertificatePdf(certificateId: string) {
  ...
  const pdfBuffer = await generateCertificatePdf({
    ...
    orgName: certificate.course?.tenant?.name ?? null,
    orgLogoUrl: certificate.course?.tenant?.logoUrl ?? null,
    orgPrimaryColor: certificate.course?.tenant?.primaryColor ?? null,
  });
...
/** Re-renders every certificate for a course — used after the template or a tenant's branding changes. */
