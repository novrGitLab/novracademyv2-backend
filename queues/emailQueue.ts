...
/**
 * Runs a background job off the request path. The work is deferred to the
 * next tick and errors are logged, never thrown — so a slow email provider
 * or certificate render can't block the HTTP response that triggered it.
 *
 * NOTE: In-process only. On a multi-instance deployment these jobs run on
 * whichever instance handled the request; that is acceptable for
 * notification emails. Replace with a real queue (BullMQ/Redis or a DB job
 * table + worker) if delivery guarantees ever require durability across
 * restarts.
 */
function defer(jobName: string, run: () => Promise<void>): void {
  setImmediate(() => {
    run().catch((err) => {
...
export const enqueueEnrollmentConfirmedEmail = (enrollmentId: string) => {
  defer("enrollment-confirmed", async () => {
...
export const enqueueCertificateIssuedEmail = (certificateId: string) => {
  defer("certificate-issued", async () => {
...
export const enqueueSegmentNotificationEmail = (userIds: string[], title: string, content: string) => {
  defer("segment-notification", async () => {
